	...
	while (1) {
		...
		if (!sockets.empty()) {
			...
			if (token != NULL) {
				...
				HANDLE hFileConfig = CreateFile(
					L"E:\\Bitdefender\\Curs9\\LastHomework\\Debug\\httpservice.config",
					GENERIC_READ,
				...
				bRead = ReadFile(hFileConfig, (LPVOID)pathToFile, 100, &dwRead, NULL);
				if (!bRead) {
