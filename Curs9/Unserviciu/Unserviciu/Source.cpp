...
#define SVCNAME TEXT("UnServiciu")
#define EventName TEXT("Global\\StopEvent")
...
VOID SvcInit(DWORD dwArgc, LPTSTR *lpszArgv)
{
	...
	*/
	...
	PSECURITY_DESCRIPTOR str = (PSECURITY_DESCRIPTOR) LocalAlloc(LPTR, SECURITY_DESCRIPTOR_MIN_LENGTH);
	InitializeSecurityDescriptor(str, SECURITY_DESCRIPTOR_REVISION);
	SetSecurityDescriptorDacl(str, TRUE, NULL, FALSE);
	SECURITY_ATTRIBUTES sa = { 0 };
    sa.nLength = sizeof(sa);
    sa.lpSecurityDescriptor = str;
...
VOID __stdcall DoStopSvc()
{
	...
	HANDLE hEvent = OpenEvent(EVENT_MODIFY_STATE, TRUE, EventName);
	if (hEvent == NULL) {
		printf("esti prost (%d)\n", GetLastError());
	}
	printf("a intrat in stop\n");
	if (!SetEvent(hEvent)) {
		printf("ERROR AT SETTING EVENT (%d)\n", GetLastError());
		return;
	}
